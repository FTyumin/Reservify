...
@section('content')
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <h1 class="text-2xl font-bold mb-6">{{ __('schedule_details') }}</h1>

        <div class="bg-white shadow rounded-lg p-6 mb-6">
            <p class="text-gray-800 font-semibold">{{ __('hotel') }}: {{ $hotel->name }}</p>
            <p class="text-gray-800 font-semibold">{{ __('room_type') }}: {{ $room->type }}</p>
            <p class="text-gray-800 font-semibold">{{ __('cleaning_date') }}: {{ $cleaningSchedule->cleaning_date }}</p>
            <p class="text-gray-800 font-semibold">{{ __('employee_id') }}: {{ $cleaningSchedule->employee_id }}</p>
        </div>
