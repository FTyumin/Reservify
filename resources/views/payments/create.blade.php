...
<div class="container mx-auto p-4">
    <h1 class="text-2xl font-semibold mb-4">{{ __('messages.create_payment_for_reservation', ['id' => $reservation->id]) }}</h1>

    <p><strong>{{ __('messages.total_price') }}:</strong> {{ $totalPrice }}€</p>
    ...
    <form action="{{ route('payments.store') }}" method="POST">
        ...
        <div class="mb-4">
            <label for="credit_card_number" class="block text-sm font-medium text-gray-700">{{ __('messages.credit_card_number') }}</label>
        ...
        <div class="mb-4">
            <label for="expiry_date" class="block text-sm font-medium text-gray-700">{{ __('messages.expiry_date') }}</label>
        ...
        <div class="mb-4">
            <label for="cvv" class="block text-sm font-medium text-gray-700">{{ __('messages.cvv') }}</label>
        ...
        <button type="submit" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">{{ __('messages.create_payment') }}</button>
    </form>
</div>
@endsection