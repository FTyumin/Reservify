...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    <h1 class="text-2xl font-bold mb-6">{{ __('messages.edit_hotel') }}</h1>

    @if ($errors->any())
        <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative mb-6" role="alert">
            <strong class="font-bold">{{ __('messages.correct_errors') }}</strong>
    ...
    <form action="{{ route('hotels.update', $hotel) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div>
            <label for="name" class="block text-sm font-medium text-gray-700">{{ __('messages.hotel_name') }}:</label>
        ...
        <div>
            <label for="rating" class="block text-sm font-medium text-gray-700">{{ __('messages.rating') }}:</label>
